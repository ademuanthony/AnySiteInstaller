...
require_once('app/code/core/helper/constant.php');
require_once('app/code/core/helper/StringMethods.php');
require_once('app/code/core/helper/db.php');
require_once('app/code/core/helper/html.php');
require_once('app/code/core/helper/Mobile_Detect.php');
require_once('app/code/core/model/system/base.php');


require_once('app/code/core/abstract/module/base.php');
require_once('app/code/core/abstract/module/controller.php');
...
class Core{
    ...
    const SETTINGS_FILE_PATH = 'app/code/core/settings.xml';
    ...
    const OPEN_USER_STATUS_PENDING_APPROVAL = 'pending';
    const OPEN_USER_STATUS_ACTIVE = 'active';
    const OPEN_USER_STATUS_INACTIVE = 'inactive';
    ...
    const OPEN_TRANSACTION_TYPE_DEPOSIT = 'Deposit';
    const OPEN_TRANSACTION_TYPE_WITHDRAWAL = 'Withdrawal';
    const OPEN_TRANSACTION_TYPE_TRANSFER_OUT = 'Transfer Out';
    const OPEN_TRANSACTION_TYPE_TRANSFER_IN = 'Transfer In';
    ...
    //transfer types
    const OPEN_TRANSFER_TYPE_FROM = 'from';
    const OPEN_TRANSFER_TYPE_TO = 'to';

    //deposit status
    const OPEN_DEPOSIT_STATUS_PENDING = 'pending';
    const OPEN_DEPOSIT_STATUS_SUCCESS = 'success';
    const OPEN_DEPOSIT_STATUS_REJECTED = 'rejected';
    ...
    public static function setCurrentRoute(Core_Model_System_Route $route){
    ...
    public static function getCurrentTheme(){
        return new Core_Model_System_Theme(self::getSystemSetting(Core::CURRENT_THEME_KEY));
    }
    ...
    public static function loadModelRegistry()
    {
        ...
        $xml=simplexml_load_file('app/code/core/model/registry.xml');

        //var_dump($xml);die();
        foreach($xml->model as $modelMeta){
            $model_meta = new Core_Model_System_ModelMeta((string)$modelMeta->key,
    ...
    private function getModelMeta($model_name){
        try {
            ...
            if(!class_exists('Core_Model_Abstract_ModelBase'))
                require_once('app/code/core/model/abstract/modelBase.php');
    ...
    public static function getPaymentMethod($_key, $loadController = false){
        return Core_Model_System_Payment::getPaymentMethod($_key, $loadController);
    }

    public static function getPaymentMethods(){
        return Core_Model_System_Payment::getPaymentMethods();
    }

    //module fields
    public static function getField($key){
        return Core_Model_System_Module::getField($key);
    }
    ...
    public static function getActionUrl($action, $controller = '', $module = '', array $param = null, array $queryString = null)
    {
        ...
        if($controller != '*') {
            if (empty($controller) && strtolower($module) == strtolower(Core::$module->name))
    ...
    public static function registerView($key, $content, $type, $position, $isFile = true){
        $view = new Core_Model_System_View($key, $content, $type, $isFile, $position);
    ...
    public static function runAction($key, array $params = []){
        foreach (Core_Model_System_Action::getAll() as $action) {
            if($action->key == $key){
                try{
        ...
        if ($handle = opendir($dir)) {
            while (false !== ($entry = readdir($handle))) {
                if ($entry != "." && $entry != "..") {
                    $names = explode('.', $entry);
                    $name = $names[0];
                    $images[$name] = $fullPart? Core::getBaseUrl().$dir.'/'.$entry : $dir.'/'.$entry;
                }
            }
            closedir($handle);
        }
        ...
        if ($handle = opendir($dir)) {
            ...
            while (false !== ($entry = readdir($handle))) {
                if ($entry != "." && $entry != "..") {
                    $names = explode('.', $entry);
                    $name = $names[0];
                    $themeImages[$name] = $fullPart? Core::getBaseUrl().$dir.'/'.$entry : $dir.'/'.$entry;
                }
            }
            closedir($handle);
        }

        $images[Core::CURRENT_THEME_KEY] = $themeImages;
        self::$images = $images;
        return self::$images;
    }
    ...
    public static function getImage($key, $fullPart = true){
        ...
        if(isset($images[Core::CURRENT_THEME_KEY][$key])) return $images[Core::CURRENT_THEME_KEY][$key];
    ...
    private function splitUrl()
    {
        ...
        if (isset($_GET['q'])) {
            ...
            //get the module
            self::$module = Core_Model_System_Module::getModule($this->module_name);
