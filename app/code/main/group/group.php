...
class Group extends Core_Abstract_Module_Controller{
    public function Index(){
        $this->data['pageTitle'] = "Groups -> ".Core::getSystemSetting(Core::SITE_NAME);
        ...
        if(isset($_REQUEST['callback'])){
            echo Core::jSonp(array('error'=>FALSE, 'message'=> 'success', 'groups'=>$this->data['groups'],
    ...
    public function add(){
        ...
        if(isset($_REQUEST['callback'])){
            echo Core::jSonp(array('error'=>$error_code != 0, 'message'=> $notification), $_REQUEST['callback']);
            exit();
        }
    ...
    public function detail($groupId, $_page = 0){
        ...
        if($group->GroupExits == TRUE){
            ...
            for($i = 0; $i <= $no ; $i++)
            {
                ...
                {
                    $link .= '<li><a href="'.Core::getActionUrl('detail', '*', 'group',
            ...
            if(isset($_REQUEST['callback'])){
                Core::jSonp(array('error'=>false, 'group'=>$group, 'contacts' =>$contacts), $_REQUEST['callback']);
                exit;
            }
            ...
            if(isset($_REQUEST['callback'])){
                Core::jSonp(array('error'=>true, 'message'=>'Group not found'), $_REQUEST['callback']);
                exit;
            }
        ...
        $this->data['pageTitle'] = "Groups | ".$this->data['group']->Name." -> ".Core::getSystemSetting(Core::SITE_NAME);
        $this->renderTemplate('body');
    }
    ...
    public function update($groupId){
        ...
        Core::redirectToAction('detail', '*', 'group', ['parameter1' => $groupId]);
    }
    ...
    public function delete($groupId){
        ...
        if(isset($_REQUEST['callback'])){
            echo Core::jSonp(array('error'=>FALSE, 'message'=> 'Group Deleted'), $_REQUEST['callback']);
        }
            ...
            if(isset($_REQUEST['contacts'])){
                ...
                if(!(empty($_FILES['uFile']['name']))){
                    ...
                    if(in_array($ext,$allowedXls) ) {
                        //excel sheet
                        require_once('app/code/core/helper/excel_reader2.php');
