 ...
 * Time: 14:28
 */
class Api extends Core_Abstract_Module_Controller{
    ...
    public function Send(){
        ...
        if(isset($_REQUEST['callback'])){
            ...
            if($_REQUEST['sender'] == '' || $_REQUEST['message'] == ''){
                ...
                $avu = ($user->Balance * 1);
                $uneeded = ($count * Core::getSystemSetting(Core::OPEN_UNITS_PER_SMS));
                ...
                if($avu < $uneeded){
                    ...
                    if($count > 0 && !$hasErro){
                        ...
                        */
                        ...
                        if(strpos(strtolower($xml), strtolower(Core::getField('Sms_Api_Success_Keyword')->value))){
        ...
        die(Core::jSonp(['message'=>$notification, 'error' => $hasErro], $_REQUEST['callback']));
    }

    public function Balance(){
        if(empty($_REQUEST['callback'])) $_REQUEST['callback'] = 'callback';
        $user = $this->checkLogin();
        die(Core::jSonp(array('balance'=>$user->Balance, 'error' => false), $_REQUEST['callback']));
    }
}