...
class Users extends Core_Abstract_Module_Controller {

    public function Index($_page = 0){
        $this->data['user'] = $this->checkLogin(Core::OPEN_ROLE_ADMIN);
        $this->data['pageTitle'] = 'Manage Users | '.Core::getSystemSetting(Core::SITE_NAME);
        $this->data['page'] = $_page;

        //########==paging==########//
        $rec_limit = 10;
        $count = $this->callModelStaticMethod('OpenSms_Model_User', 'Count', [0 => Core::OPEN_USER_STATUS_ACTIVE]);
        $no = ($count/$rec_limit);
        ...
        for($i = 0; $i <= $no ; $i++)
        {
            ...
            {
                $link .= '<li><a href="'.Core::getActionUrl('index', 'users', 'admin',
    ...
    public function manage($_loginId){
        //die($_loginId);
        $this->data['user'] =  $this->checkLogin(Core::OPEN_ROLE_ADMIN);
        $this->data['pageTitle'] = 'Manage user | '.Core::getSystemSetting(Core::SITE_NAME);
