...
class Transactions extends  Core_Abstract_Module_Controller{
    ...
    public  function Index($_page = 0){
        ...
        for($i = 0; $i <= $no ; $i++)
        {
            ...
            {
                $link .= '<li><a href="'.Core::getActionUrl('index', 'transactions', 'admin',
        ...
        $this->data['pageTitle'] = 'Transactions | ' . Core::getSystemSetting(Core::SITE_NAME);
        $this->renderTemplate();
    }
    ...
    public function Add(){
        ...
        if(isset($_POST['Amount'])){
            ...
            $result = ($tran->Status == Core::OPEN_TRANSACTION_STATUS_COMPLETED)? $tran->Commit():$tran->Save();

            if($result != true){
                $this->setError($result, 'add_transaction');
                if(!empty($_REQUEST['returnUrl'])) {
                    Core::redirect($_REQUEST['returnUrl']);
                }
            }else{
                //if completed send notification
                if(Core::OPEN_TRANSACTION_STATUS_COMPLETED == $tran->Status){
                    $this->sendSmsAlert($tran);
                }
                $this->setNotification('Transaction saved', 'add_transaction');
                if(empty($_REQUEST['returnUrl'])) Core::redirectToAction('Index');
                Core::redirect($_REQUEST['returnUrl']);
            }
    ...
    public function Manage($id){
        $this->data['user'] = $this->checkLogin();
        $this->data['transaction'] = Core::loadModel('OpenSms_Model_Transaction', [0 => $id]);
        $this->data['pageTitle'] = 'Manage Transaction';
        $this->data['returnUrl'] = isset($_REQUEST['returnUrl'])?$_REQUEST['returnUrl']:'';

        $this->data['payments'] = Core::getPaymentMethods();
        $this->data['pageTitle'] = 'Manage Transaction | ' . Core::getSystemSetting(Core::SITE_NAME);
        $this->renderTemplate();
    }
    ...
    public function Update(){
        ...
        if(isset($_POST['Amount'])){
            ...
            if($result != true){
                ...
                if(!empty($_REQUEST['returnUrl'])) {
                    Core::redirect($_REQUEST['returnUrl']);
                }else Core::redirectToAction('Index');
            }else{
                $this->setNotification($tran->Status == Core::OPEN_TRANSACTION_STATUS_COMPLETED?'Transaction saved and committed'
    ...
    public function Delete($id){
        ...
        $tran = Core::loadModel('OpenSms_Model_Transaction', [0 => $id]);
        if(!isset($tran->LoginId)) return 'Invalid transaction Id';
        $result = $tran->Delete();
        if(!$result) $this->setError($result, 'delete_transaction');
        if(empty($returnUrl)) Core::redirectToAction('Index');
        Core::redirect($returnUrl);
    }
