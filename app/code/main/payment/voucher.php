...
class Voucher extends Core_Abstract_Module_Controller{
    public function Pay(){
        Core::redirectToAction('index', 'voucher', 'payment');
    }

    public function Index(){
        $this->data['user'] = $this->checkLogin();
        $this->data['transaction'] = $this->data['user']->GetLastTransaction();
        if($this->data['transaction']->Status == Core::OPEN_TRANSACTION_STATUS_COMPLETED){
            $this->setError("No pending transaction found", "voucher_index");
            Core::redirectToAction('Index', 'Recharge', 'Account');
        }
        ...
        if(isset($_POST['pin'])){
            if(empty($_POST['pin']) || empty($_POST['serialNumber'])){
                ...
                $card = Core::loadModel('OpenSms_Model_Card', [0 => $_POST['serialNumber'], 1 => $_POST['pin']]);
                if($card->IsValid) {
                    $result = $card->Load($this->data['user']->LoginId);
                    if ($result['success'] != true) {
                        $this->setError($result['message'], 'voucher_index');
                    } else {
                        $this->data['transaction']->Status = Core::OPEN_TRANSACTION_STATUS_COMPLETED;
