<?php
class Core_Abstract_Module_Controller extends Core_Abstract_Module_Base{
   ...
   */
   protected function getThemeName(){
       if(Core::getCurrentRoute()->controller == 'install') return 'default';
   ...
   protected function getTemplatePath($block_name){
       ...
       */
       ...
       $sFileName = Core::DESIGN_PATH.$this->getThemeName().'/template/'.(string)$this->getLayout()->{$block_name}['template'];
       if(!file_exists($sFileName))
           $sFileName = Core::DESIGN_PATH.'default'.'/template/'.(string)$this->getLayout()->{$block_name}['template'];
       return $sFileName;
   }

   protected function renderTemplate($block_name = 'body', $required = true){
       if($block_name == 'body') Core::runAction(Core::BEFORE_RENDER_ACTION);
    ...
    protected function getCurrentUri(){
        return Core::getActionUrl(Core::getCurrentRoute()->action);
    }

    protected function isCurrentUri($action, $controller = '', $module = ''){
        return Core::getActionUrl($action, $controller, $module) == $this->getCurrentUri();
    }
    ...
    protected function renderStyle($block_name = 'head'){
        ...
        foreach($scriptElements as $script){
            $sFileName = Core::DESIGN_PATH.$this->getThemeName().'/assets/'.(string)$script['href'];
            if(!file_exists($sFileName)){
                $fileName = Core::DESIGN_PATH . 'default' . '/assets/' . (string)$script['href'];
        ...
        if(is_array($scripts)) {
            foreach ($scripts as $sFile) {
                echo html_entity_decode("<link href='".Core::getBaseUrl().$sFile."' rel='stylesheet'/>");
            }
        }else {
            echo html_entity_decode("<link href='".Core::getBaseUrl().$scripts."' rel='stylesheet'/>");
        }
    ...
    protected function renderScript($block_name = 'head'){
        ...
        foreach($scriptElements as $script){
            $sFileName = Core::DESIGN_PATH.$this->getThemeName().'/assets/'.(string)$script['src'];
            if(!file_exists($sFileName)){
                $fileName = Core::DESIGN_PATH . 'default' . '/assets/' . (string)$script['src'];
                if (file_exists($fileName)) $sFileName = $fileName;
            }
            $scripts[] = $sFileName;
        }
        ...
        if(is_array($scripts)) {
            foreach ($scripts as $sFile) {
                echo html_entity_decode("<script src='".Core::getBaseUrl().$sFile."' type='text/javascript'></script>");
            }
        }else {
            echo html_entity_decode("<script src='".Core::getBaseUrl().$scripts."' type='text/javascript'></script>");
        }
    }

    protected function registerView($key, $content, $type, $position, $isFile = true){
        Core::registerView($key, $content, $type, $position, $isFile);
    }

    protected function renderSpecialView($position){
        $views = Core::getViews($position);
        foreach($views as $view){
            switch($view->type){
                case Core::VIEW_TYPE_STYLE:
                    echo html_entity_decode("<link href='".Core::getBaseUrl().Core::DESIGN_PATH.
                        $view->content."' rel='stylesheet'/>");
                    break;
                case Core::VIEW_TYPE_SCRIPT:
                    echo html_entity_decode("<script src='".Core::getBaseUrl().Core::DESIGN_PATH.
                        $view->content."' type='text/javascript'></script>");
                    break;
                case Core::VIEW_TYPE_HTML:
                case Core::VIEW_TYPE_RAW:
                    if($view->isFile) require_once(Core::DESIGN_PATH.$view->content);
                    else echo (($view->type==Core::VIEW_TYPE_HTML)?html_entity_decode($view->content):$view->content);
                    break;
            }
        }
    }
    ...
    protected function renderContent($key, array $htmlAttributes = array()){
        $content = Core::getContent($key);
        if($content->Key){
            $attributes = "";
            $class_added = false;
            //$content->Host = $content->Type == OpenSms::VIEW_TYPE_HTML? 'div': $content->Host;
            $edit_class = $content->Type == Core::VIEW_TYPE_HTML? 'editable':'raw_editable';
   ...
   protected function redirect($url){
    ...
    protected function redirectToAction($action, $controller = '', $module = '', array $routeParam = null){
        Core::redirectToAction($action, $controller, $module, $routeParam);
    }

    protected function setError($message, $key)
    {
        Core::setError($message, $key);
    }

    protected function setNotification($message, $key){
        Core::setNotification($message, $key);
    }
    ...
    protected function requestIsAuthenticated(){
        return Core::requestIsAuthenticated();
    }

    public function checkLogin($role = ''){
        $user = Core::checkLogin($role);
        $this->data['user'] = $user;
        return $user;
    }

    protected function isUserInRole($role){
        return Core::isUserInRole($role);
    }

    //image upload
    public function uploadImage($fileKey, $rootFolder, &$img_name){

        require('app/code/core/helper/ImageEditor.php');
	     ...
	     "png") && ($extension != "gif")) 
...

			    {
                    ...
                    */
                    ...
                    return array('success' => $noError, 'newName' => $newname, 'errorMessage' => $error_msg);
	            }
        }
    ...
    protected function sendSmsAlert(Core_Model_Transaction $transaction){
        $user = $user = $this->loadModel('OpenSms_Model_User', array(0 => $transaction->LoginId));
    ...
    protected function sendMessage($sender, $message, $recepients, $loginId = '', $sendOnDate = '2/2/2'){
        ...
        //take away the 1st and last comma
         ...
         $avu = ($user->Balance * 1);
         $unitsNeeded = ($count * Core::getSystemSetting(Core::OPEN_UNITS_PER_SMS));
         ...
         if($avu < $unitsNeeded){
             ...
             if($count > 0 && !$hasError){
                 ...
                 $url = Core::getField('Sms_Send_Api')->value;

                 //replace username, password, senderId, message, recipients, sendOnDate
                 $url = str_replace('@username@', Core::getField('Sms_Api_Username')->value, $url);
                 $url = str_replace('@password@', Core::getField('Sms_Api_Password')->value, $url);
                 ...
                 if(strpos(strtolower($xml), strtolower(Core::getField('Sms_Api_Success_Keyword')->value)) !== false){
                     $user->Balance -= ($count * Core::getSystemSetting(Core::OPEN_UNITS_PER_SMS));
                     $user->Save();
    ...
    protected function sendEmail($subject, $email, $body, $from, $isHtml = false)
    {
        ...
        if(!class_exists('PHPMailer')){
            include("app/code/core/helper/smtp/class.phpmailer.php"); //you have to upload class files "class.phpmailer.php" and "class.smtp.php"
        }
        $mail = new PHPMailer();
    ...
    public function deductAccount($qnt){
        if(!class_exists('Core_Model_Login'))
            $this->loadModel('login');
        $token = new Core_Model_Login();
        if(empty($loginId)){
