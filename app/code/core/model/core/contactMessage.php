...
class Core_Model_ContactMessage extends Core_Model_Abstract_ModelBase{
    ...
    public function getById($id = -1){
        $this->Id = $id;
        $this->Status = Core::CONTACT_MESSAGE_STATUS_NEW;
    ...
    public function getTableName(){
        return Core::getTableName('contactMessage');
    }
    ...
    public static function GetMessages($status = Core::CONTACT_MESSAGE_STATUS_NONE, $offset = 0, $limit = 0)
    {
        $obj = new Core_Model_ContactMessage();
        $sql = "select * from " . $obj->getTableName() . ($status == Core::CONTACT_MESSAGE_STATUS_NONE? " ": " where status = '$status'");
        if ($limit > 0) {
