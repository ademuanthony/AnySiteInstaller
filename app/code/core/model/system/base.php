...
class Core_Model_System_Module_Field{
...
class Core_Model_System_ModelMeta{
...
class Core_Model_System_Payment{
    ...
    public static function getPaymentMethods(){
        ...
        $modules = Core_Model_System_Module::getModules();
...
class Core_Model_System_Module{
    ...
    */
    ...
    private function load($fileName){
        ...
        foreach($module_xml->routes->route as $route){
            $route_model = new Core_Model_System_Route((string)$route->uri, (string)$route->controller,
        ...
        foreach($module_xml->fields->field as $field){
            ...
            $field_model = new Core_Model_System_Module_Field((string)$field->label, (string)$field->key, (string)$field->type,
        ...
        foreach($module_xml->payments->payment as $payment){
            $payment_model = new Core_Model_System_Payment((string)$payment->label, (string)$payment->key,
        ...
        if(isset($module_xml->actions->action))
            foreach($module_xml->actions->action as $action_xml){
                $action = new Core_Model_System_Action((string)$action_xml->key, (string)$action_xml->method,
    ...
    */
    public  static function getModule($_name){
        ...
        return !empty(self::$modules[$_name]) ? self::$modules[$_name] : new Core_Model_System_Module($_name);
    }
...
class Core_Model_System_Action{
...
class Core_Model_System_Theme_Field{
...
class Core_Model_System_Theme{
    ...
    private function load($key){
        $fileName = Core::DESIGN_PATH.$key.'/theme.xml';
        if(!file_exists($fileName)) return NULL;
        ...
        if(isset($theme_xml->fields->field))
            foreach($theme_xml->fields->field as $field){
                ...
                $field_model = new Core_Model_System_Theme_Field((string)$field->label, (string)$field->key, (string)$field->type,
    ...
    public function getSettingsFile(){
        return Core::DESIGN_PATH.$this->key.'/theme.xml';
    }
    ...
    public static function getAll(){
        ...
        $directories = glob(Core::DESIGN_PATH . '*' , GLOB_ONLYDIR);
        foreach($directories as $dir){
            $key = explode('/',$dir)[2];
            if(!isset(self::$collection[$key])){
                new Core_Model_System_Theme($key);
            }
        }
